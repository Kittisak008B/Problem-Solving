...
# Constraints:  formula consists of English letters, digits, '(', and ')'.   formula is always valid.
    ...
    def countOfAtoms(self, formula: str) -> str:
        ...
        while i < len(formula) :
            if formula[i] == '(' :
                ...
                for ele in cur_dict :
                    stack[-1][ele] += cur_dict[ele] * count 
            else :
                element = formula[i]
                count = ''
            ...
            i += 1
        ans = ''
        for element in sorted(stack[-1].keys()) :
            count = '' if stack[-1][element] == 1 else str(stack[-1][element])
            ans += element + count     
        return ans  
'''
stack = [{Mg:1} , {O:1 , H:1}]
