...
# Example 1:
# Input: positions = [5,4,3,2,1], healths = [2,17,9,15,10], directions = "RRRRR"   Output: [2,17,9,15,10]
...
#               and since both have the same health, they will be removed from the line. Next, robot 3 and robot 4 will collide and since robot 4's health is smaller, 
#               it gets removed, and robot 3's health becomes 15 - 1 = 14. Only robot 3 remains, so we return [14].

# Example 3:
# Input: positions = [1,2,5,6], healths = [10,10,11,11], directions = "RLRL"   Output: []
...
# Constraints: 1 <= positions.length == healths.length == directions.length == n <= 105      1 <= positions[i], healths[i] <= 109
# directions[i] == 'L' or directions[i] == 'R'      All values in positions are distinct

class Solution:
    def survivedRobotsHealths(self, positions: List[int], healths: List[int], directions: str) -> List[int]:
        bot = []
        for i in range(len(positions)) :
            bot.append([positions[i] , healths[i] , directions[i] , i])
        ...
        for x in bot :
            if x[2] == 'R' : 
                ...
                while stack and stack[-1][2] == 'R' and stack[-1][1] > 0 and x[1] > 0 :
                    if stack[-1][1] > x[1] :
                        stack[-1][1] -= 1
                        x[1] = 0 
                    elif stack[-1][1] == x[1] :
        ...
        survived_bot = [x[1] for x in stack]
        return survived_bot
