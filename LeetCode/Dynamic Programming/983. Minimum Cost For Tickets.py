...
# On day 31, you bought a 1-day pass for costs[0] = $2 which covered day 31.  In total, you spent $17 and covered all the days of your travel.
...
class Solution:
    def mincostTickets(self, days: List[int], costs: List[int]) -> int:
        ...
        def dfs(i) :
            ...
            if i in dp :
                return dp[i]
            daypass = costs[0] + dfs(i + 1)
            weekpass = costs[1] + dfs(bisect.bisect_left(days , days[i] + 7))
            monthpass = costs[2] + dfs(bisect.bisect_left(days , days[i] + 30))
            dp[i] = min(daypass , weekpass , monthpass)
            return dp[i]
        return dfs(0)
