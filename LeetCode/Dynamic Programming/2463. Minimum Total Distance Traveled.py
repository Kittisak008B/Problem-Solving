...
class Solution:
    def minimumTotalDistance(self, robot: List[int], factory: List[List[int]]) -> int:
        dp = {}
        def dfs(bot , fac_id , limit) :
            if bot >= len(robot) : #all bots have been repaired
                return 0
            if fac_id >= len(factory) : #check all factories
                return float('inf')
            if (bot , fac_id , limit) in dp :
                return dp[(bot , fac_id , limit)]
            skip_repair = dfs(bot , fac_id +1 , 0) #repair this bot at a different factory
            repair = float('inf')
            if factory[fac_id][1] > limit :
                repair = abs(factory[fac_id][0] - robot[bot]) + dfs(bot+1, fac_id , limit+1)
            dp[(bot , fac_id , limit)] = min(skip_repair, repair)
            return dp[(bot , fac_id , limit)]
