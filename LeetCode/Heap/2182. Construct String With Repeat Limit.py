...
class Solution:
    def repeatLimitedString(self, s: str, repeatLimit: int) -> str:
        ...
        for char in s :
            d[char] += 1
        max_heap = [ (-ord(c) , count) for c , count in d.items()]
        heapq.heapify(max_heap)
        res = []
        while max_heap :
            char , count = heapq.heappop(max_heap)
            char = chr(-char)
            cur_count = min(repeatLimit , count)
            res.append(char * cur_count)
            if count - cur_count > 0 and max_heap :
                next_char , next_count = heapq.heappop(max_heap)
                next_char = chr(-next_char)
                res.append(next_char)
                heapq.heappush(max_heap , (-ord(char) , count - cur_count))
                if next_count > 1 :
