...
impl NumberContainers {
    ...
    fn change(&mut self, index: i32, number: i32) {
        self.index_to_number.insert(index, number); //update mapping
        self.number_to_indices.entry(number).or_insert_with(BinaryHeap::new).push(Reverse(index));//insert index into min-heap
    }
    fn find(&mut self, number: i32) -> i32 {
        if let Some(heap) = self.number_to_indices.get_mut(&number) {
            while let Some(&Reverse(index)) = heap.peek() {
                if self.index_to_number.get(&index) == Some(&number) {
                    return index;
                }
                heap.pop();
            }
